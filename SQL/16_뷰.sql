...
/*
    # 뷰 (View)
        ...
        CREATE [OR REPLACE] [FORCE | NOFORCE] VIEW 뷰이름 [(별칭, 별칭 ...)] 
            ...
            - CREATE OR REPLACE : 이미 존재하는 VIEW이름을 사용하면 해당 VIEW를 수정한다. (참조형 변수 갈아치우는 느낌)
            - FORCE : 기본 테이블의 존재 여부에 상관없이 뷰를 생성한다.
            - WITH CHECK OPTION : 해당 뷰를 통해 볼 수 있는 범위 내에서만 UPDATE와 INSERT가 가능해진다
            - WTIH READ ONLY : 해당 뷰는 SELECT만 가능하며 INSERT/UPDATE/DELETE를 할 수 없게된다.
*/
...
CREATE OR REPLACE VIEW EMP_MGR_VIEW(사원번호, 이름, 부서번호, 매니저번호, 매니저이름) AS
SELECT e1.employee_id, e1.last_name, e1.department_id, e1.manager_id, e2.last_name
...
-- FORCE : 기본 테이브이 존재하지 않더라도 강제로 뷰를 생성한다.
...
-- 테이블이 나중에 생성되면 작동할 뷰를 미리 만들어놓을 때 사용
...
/*
    ...
    # 하나의 테이블만 이용하는 뷰를 단순 뷰라고 부르고, 
        조인을 통해 두개 이상의 테이블을 이용하는 뷰를 복합 뷰라고 부른다.
*/
---------------------------------------------------------------------------------------
